DELIMITER //
DROP PROCEDURE IF EXISTS createObjectRules;
CREATE PROCEDURE `createObjectRules`()
BEGIN
  ...
  DECLARE v_conf_threshold INTEGER DEFAULT 0.8;
  ...
  loop_objs: LOOP
    ...
    INSERT INTO rulecount_objects (head, tail, count, confidence, confidence_distinct) (
      SELECT t1.obj_id AS head, t2.obj_id AS tail, COUNT(DISTINCT t1.sub_id) AS COUNT, 
      ...
      WHERE t1.obj_id = v_obj_id AND t1.obj_id <> t2.obj_id AND sub_exclude = 0
      GROUP BY head, tail
      HAVING count > 1 AND confidence_distinct >= v_conf_threshold AND confidence_distinct < 1
    );
  ...
  SELECT count(*) as 'total compressed rules' FROM rulecount_objects;
  SELECT sum(c) as 'total rules' FROM rulecount_objects;
